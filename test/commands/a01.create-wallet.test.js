...
describe('create-wallet', () => {
  ...
  it('should create a mainnet wallet file with the given name', async () => {
    ...
    if (process.env.TEST !== 'unit') { createWallet.BITBOX = new config.BCHLIB({ restURL: config.MAINNET_REST }) }
    ...
    if (process.env.TEST !== 'unit') { assert.equal(walletData.rootAddress.indexOf('bitcoincash') > -1, true) }
  })
  ...
  it('should create a testnet wallet file with the given name', async () => {
    // Use the real library if this is not a unit test.
    if (process.env.TEST !== 'unit') { createWallet.BITBOX = new config.BCHLIB({ restURL: config.TESTNET_REST }) }
